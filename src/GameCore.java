					...
					try {
						...
						Thread nightGhoul = new Thread(new Runnable() {
							...
							public void run() {
								...
								String ghoulName = "[" + ghoul.getTrueName() + "]";
								GameCore.this.broadcast(room,
										"You see a night Ghoul named " + ghoulName + " appear in this room");
								while (!isDay) {
									try {
										// Ghoul move in each 12-17 seconds.
										...
										GameCore.this.ghoulWander(ghoul, room);
										GameCore.this.broadcast(room,
												"You see a night Ghoul named " + ghoulName + " leave in this room");
										room = map.findRoom(ghoul.getRoom());
										GameCore.this.broadcast(room,
												"You see a night Ghoul named " + ghoulName + " enter in this room");
									} catch (InterruptedException ex) {
										Logger.getLogger(GameObject.class.getName()).log(Level.SEVERE, null, ex);
									}
								}
	...
	public String bribeGhoul(String playerName, String item) {
		...
		if (room.hasGhoul) {
			...
			if (giveAble) {
				...
				String message = "Ghoul [" + ghoul.getTrueName() + "] gets " + item + ", "
						+ "and its anger level decreases to " + angryLv + ".";
				return message;
			} else {
		...
		if (player != null) {
			...
			if (angerLvl >= 7) {
				...
				player.getReplyWriter()
						.println("The ghoul is planning to drag you! You have 5 seconds to FIGHT it w/ R-P-S!");
